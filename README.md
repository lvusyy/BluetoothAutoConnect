...
本程序**支持蓝牙音频设备（耳机、音箱）的自动连接**。

**工作原理**：

- 程序通过 `BluetoothAuthenticateDeviceEx` API 发送一个认证请求，这会**触发 Windows 重新建立与已配对设备的连接**，类似于在设置中手动点击“连接”。
- 发送请求后，程序会**耐心等待最多 20 秒**，持续检查设备状态，直到连接成功。
- 这个过程完全在后台进行，**无需任何用户交互**。

**总结**：只要您的音频设备已开机并在范围内，程序就能自动为您连接，真正实现后台值守。
